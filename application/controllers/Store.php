...

{
	...
	{				
		...
		try {
			$ratings_load = @loadFile("http://127.0.0.1:4002/ob/ratings/" . $peerID."?usecache=true");
			if ($ratings_load !== FALSE) {
				$ratings = json_decode($ratings_load);

				foreach($ratings->ratings as $r) {
				    $rating_load = @loadFile("http://127.0.0.1:4002/ob/rating/" . $peerID. "/" . $r . "?usecache=true");
		...
		foreach($listing_ratings as $r) {
			$review_load = @loadFile("http://127.0.0.1:4002/ipfs/" . $r);
		...
		$this->load->view('header', array(
			...
			'page_image' => "http://127.0.0.1:4002/ob/images/".$listing_image_hash
		));
	...
	function listings($peerID, $category = "All")
	{
		...
		$this->load->view('header', array(
			...
			'page_image' => "http://127.0.0.1:4002/ob/images/".$image_hash
		));
	...
	function followers($peerID)
	{
		...
		$profile = get_profile($peerID);
		$header_image = isset($profile->headerHashes);
		$followers_load = loadFile("http://127.0.0.1:4002/ob/followers/" . $peerID."?usecache=true");
	...
	function following($peerID)
	{
		...
		$header_image = isset($profile->headerHashes);
		$followers_load = loadFile("http://127.0.0.1:4002/ob/following/" . $peerID."?usecache=true");
		$followers = json_decode($followers_load);
		$data = array(
